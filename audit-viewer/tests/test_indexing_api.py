...
# Ensure audit_viewer is importable
...
def indexing_client(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("INDEXING_DB_PATH", str(tmp_path / "indexing.db"))
    ...
    from audit_viewer.api import app as app_mod
    from audit_viewer.api import deps as deps_mod
    importlib.reload(deps_mod)
    importlib.reload(app_mod)

    from audit_viewer.api import indexing as idx_mod
...
def test_delete_partial_failure_returns_500(indexing_client, monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "audit_viewer.indexing_backends", mock_backends)
    monkeypatch.setattr(audit_viewer, "indexing_backends", mock_backends, raising=False)
