...
# ---------------------------------------------------------------------------
try:
    from audit_viewer.api import indexing as _indexing_mod  # noqa: F401
...
pytestmark = pytest.mark.skipif(
    ...
    reason="audit_viewer.api.indexing not yet available — phases #4/#5 pending",
)
...
from audit_viewer import config as av_config
...
def tmp_db(tmp_path, monkeypatch):
    """Point INDEXING_DB_PATH at a fresh temp SQLite file."""
    db_path = tmp_path / "indexing_test.db"
    monkeypatch.setattr(av_config, "INDEXING_DB_PATH", db_path)
    monkeypatch.setenv("INDEXING_DB_PATH", str(db_path))
...
def client(tmp_db):
    ...
    from audit_viewer.api.app import create_app
