...
class GraphAdapter(SearchAdapter):
    ...
    async def search_by_graph(
        ...
        # 第一步：全文检索种子节点
        seed_nodes = await fulltext_search_nodes(driver, terms, limit=20)
        if not seed_nodes:
            return []
        ...
        # 第二步：扩展到 File 节点
        neighbor_results = await expand_neighbors(driver, seed_ids, max_hops=2)

        if not neighbor_results:
            return []

        # 计算最大匹配数（用于归一化）
        max_match_count = max(len(r["anchor_nids"]) for r in neighbor_results) if neighbor_results else 1
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            async with driver.session() as session:
                ...
                has_indexes = (
                    "symbol_name_idx" in index_names
                    or "doc_entity_idx" in index_names
                )
                return has_indexes
        except Exception as exc:
