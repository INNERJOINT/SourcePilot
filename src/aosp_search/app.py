...
async def retrieval(body: RetrievalRequest, request: Request):
    ...
    # 2.1 从 metadata_condition 解析 lang 和 branch 过滤条件
    lang_filter = None
    branch_filter = None
    if body.metadata_condition and body.metadata_condition.conditions:
        for cond in body.metadata_condition.conditions:
            if isinstance(cond, dict):
                field = cond.get("name", "")
                value = cond.get("value", "")
                if field == "lang" and value:
                    lang_filter = value
                elif field == "branch" and value:
                    branch_filter = value
