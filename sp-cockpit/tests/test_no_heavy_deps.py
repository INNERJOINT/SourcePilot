"""Verify sp-cockpit venv does NOT pull in heavy indexer SDK deps.

pymilvus and neo4j-driver must live only inside indexer containers,
never in the sp-cockpit Python environment (R5 from plan).
"""
import subprocess
import sys
...
def test_pymilvus_not_installed():
    ...
    assert "pymilvus" not in pkg_names, (
        "pymilvus found in sp-cockpit venv — it must stay inside the dense-indexer container"
    )
...
def test_neo4j_not_installed():
    ...
    assert "neo4j" not in pkg_names, (
        "neo4j driver found in sp-cockpit venv — it must stay inside the graph-indexer container"
    )
