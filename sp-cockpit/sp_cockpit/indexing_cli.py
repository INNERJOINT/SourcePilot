...
Usage:
    python -m sp_cockpit.indexing_cli start --repo-path /path/to/repo --backend dense [--log-path /path/to.log]
    python -m sp_cockpit.indexing_cli finish --job-id 42 --status success --exit-code 0
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m sp_cockpit.indexing_cli",
        description="Indexing job lifecycle CLI",
    )
    parser.add_argument(
        "--api-url",
        default=os.getenv("INDEXING_API_URL", "http://localhost:9100"),
        help="Base URL of sp-cockpit API (default: http://localhost:9100)",
    )
