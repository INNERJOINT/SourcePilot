...
main() {
  ...
  HTTP_CODE=$("$CURL_BIN" -s --max-time "$TIMEOUT" \
    ...
    -d "$("$JQ_BIN" -nc --arg q "$QUERY" '{query: $q, top_k: 5}')" \
  ...
  RESULT_COUNT=$("$JQ_BIN" 'length' "$RESP" 2> /dev/null || echo "?")
  echo "HTTP 200 — returned $RESULT_COUNT results"

  # ─── Check whether results contain dense source ──────────────────
  DENSE_COUNT=$("$JQ_BIN" '[.[] | select(.source == "dense")] | length' "$RESP" 2> /dev/null || echo "0")
  ZOEKT_COUNT=$("$JQ_BIN" '[.[] | select(.source != "dense")] | length' "$RESP" 2> /dev/null || echo "0")
  ...
  if [[ ! -f "$AUDIT_LOG" ]]; then
    ...
    if [[ -n "$DENSE_STAGE" ]]; then
      records=$(echo "$DENSE_STAGE" | "$JQ_BIN" -r '.stage_result.records_count // 0' 2> /dev/null || echo "?")
      echo "[PASS] dense_search stage was triggered (records_count=$records)" >&2
    else
      echo "[FAIL] dense_search stage not found in audit.log" >&2
    ...
    grep "$TRACE_ID" "$AUDIT_LOG" | "$JQ_BIN" -r '"  " + .stage + " → " + (.stage_result // {} | tostring | .[0:80])' 2> /dev/null || true
  fi
