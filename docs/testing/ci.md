...
- **Automation surface that exists today:**
  ...
  - `sp-cockpit/pyproject.toml` declares `pytest`, `pytest-asyncio`, `httpx`,
...
jobs:
  ...
  sp-cockpit:
    name: SourcePilot Cockpit
    runs-on: ubuntu-latest
    defaults:
      run:
        working-directory: sp-cockpit
    steps:
      - uses: actions/checkout@v4
      - uses: actions/setup-python@v5
        with:
...
```makefile
.PHONY: test test-unit test-integration test-e2e test-mcp test-sp-cockpit smoke cov

test: test-unit test-integration test-e2e test-mcp test-sp-cockpit
...
```bash
(cd sp-cockpit && pytest --cov=sp_cockpit --cov-report=term-missing)
```
...
- `sp-cockpit/data/audit.db` (SQLite the smoke script polled)
...
| Audit DB | Real `sp-cockpit/data/audit.db` if running locally | Tempfile fixtures only (sp-cockpit test); not relevant for SourcePilot/MCP unit jobs |
