...
def multi_project_client(monkeypatch):
    ...
    monkeypatch.setattr(cp, "_projects_cache", [PROJECT_AOSP, PROJECT_T2])
...
def test_search_project_aosp_calls_zoekt_6070(multi_project_client):
    """POST /api/search project=aosp_project hits zoekt on :6070, not :6071."""
    with respx.mock:
        route_aosp = respx.get("http://localhost:6070/search").mock(
        ...
        resp = multi_project_client.post(
            "/api/search", json={"query": "Foo", "project": "aosp_project"}
        )

    assert resp.status_code == 200
    assert route_aosp.called
