...
PROJECT_AOSP = ProjectConfig(
    name="aosp_project",
    source_root="/src/aosp",
    repo_path="/repo/aosp",
    index_dir="/idx/aosp",
    zoekt_url="http://zoekt-aosp:6070",
    dense_collection_name="aosp_code_project",
    project_type="aosp",
)
...
PROJECTS = [PROJECT_AOSP, PROJECT_FEISHU]
...
class TestIsZoektProject:
    def test_aosp_project_is_zoekt(self, monkeypatch):
        ...
        assert gw._is_zoekt_project("aosp_project") is True
    ...
    def test_default_project_is_zoekt(self, monkeypatch):
        ...
        # Default project is aosp_project (first in list), which has zoekt_url
...
class TestGetDenseAdapterFeishuIsolation:
    def test_aosp_adapter_uses_aosp_collection(self, monkeypatch):
        _patch_projects(monkeypatch)
        import gateway.gateway as gw
        ...
        with patch("adapters.dense.DenseAdapter", side_effect=_fake_dense):
            adapter = gw._get_dense_adapter("aosp_project")

        assert adapter.collection_name == "aosp_code_project"
    ...
    def test_aosp_and_feishu_adapters_are_different_instances(self, monkeypatch):
        _patch_projects(monkeypatch)
        import gateway.gateway as gw
        ...
        with patch("adapters.dense.DenseAdapter", side_effect=_fake_dense):
            ace_adapter = gw._get_dense_adapter("aosp_project")
...
class TestFeishuSearchRouting:
    ...
    async def test_aosp_search_does_not_return_feishu_results(self, monkeypatch):
        """When project=aosp_project, Feishu results must not appear."""
        _patch_projects(monkeypatch)
        import gateway.gateway as gw
        ...
        with respx.mock:
            respx.get("http://zoekt-aosp:6070/search").mock(
                return_value=httpx.Response(200, json=zoekt_hit)
            )
            results = await gw.search(query="class Foo", project="aosp_project")
