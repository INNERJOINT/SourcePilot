...
def test_yaml_minimal(tmp_path):
    ...
    p = projects[0]
    assert p["name"] == "aosp_project"
    assert p["source_root"] == "/opt/aosp/aosp_project"
    assert p["collection_name"] == "aosp_code_aosp_project"
...
def test_yaml_two_projects(tmp_path):
    ...
    assert {p["name"] for p in projects} == {"aosp_project", "aosp15"}
...
def test_fallback_default_source_root(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(pc, "_PROJ_ROOT", tmp_path)
    projects = pc.load_projects()
    assert projects[0]["source_root"] == "/opt/aosp/aosp_project"
