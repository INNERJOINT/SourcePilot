...
def main():
    ...
    parser.add_argument(
        "--source-dir", default="/opt/aosp/aosp_project/frameworks/base",
    ...
    parser.add_argument(
        "--batch-size", type=int, default=int(os.environ.get("EMBEDDING_BATCH_SIZE", "128")),
        help="Embedding batch size (default: 128)",
    )
    parser.add_argument(
        "--concurrency", type=int,
        default=int(os.environ.get("EMBEDDING_CONCURRENCY", "32")),
        help="Concurrent embedding requests (default: 32)",
    )
    parser.add_argument(
        "--project-name", default=None,
        help="Project name for collection naming (e.g. 'aosp_project' → collection 'aosp_code_aosp_project')",
    )
